# downloader/playlist.py

from yt_dlp import YoutubeDL
from downloader.save_location import get_download_path
from downloader.quality import get_format_selector
import os

def download_playlist(url, progress_callback, done_callback):
    def hook(d):
        if d['status'] == 'downloading':
            downloaded = d.get('downloaded_bytes', 0)
            total = d.get('total_bytes') or d.get('total_bytes_estimate')

            if total:
                percent_complete = (downloaded / total) * 100
                progress_callback(percent_complete / 100, f"{int(percent_complete)}%")
        elif d['status'] == 'finished':
            progress_callback(1.0, "100%")

    output_path = os.path.join(get_download_path(), '%(playlist)s/%(title)s.%(ext)s')

    ydl_opts = {
    'format': get_format_selector(),
    'outtmpl': output_path,
    'progress_hooks': [hook],
    'quiet': True,
    'merge_output_format': 'mkv',  # ensure ffmpeg combines it correctly
}


    try:
        with YoutubeDL(ydl_opts) as ydl:
            ydl.download([url])
        done_callback("✅ Playlist Downloaded")
    except Exception as e:
        done_callback(f"❌ Error: {str(e)}")
